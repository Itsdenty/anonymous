...
(function($){
	...
	// $("head").prepend("<link id='semantic_style' rel='stylesheet' href='http://localhost:8000/Semantic-UI-CSS-master/semantic.min.css' type='text/css'>");
	$("head").prepend("<link id='semantic_style' rel='stylesheet' href='http://localhost:8000/Semantic-UI-CSS-master/semantic-optin.css' type='text/css'>");
	// $("head").prepend("<script src='http://localhost:8000/Semantic-UI-CSS-master/semantic.min.js' type='text/javascript'></script>");
	...
	$.ajax({
		url: 'http://localhost:8000/api/v1/formdata/' + form_id,
		method: 'get',
		success: function(data){
			...
			if(formdata['form_type'] == 'popover' || (formdata['form_type'] == 'poll' && formdata['poll_type'] == 'popover') || (formdata['form_type'] == 'quiz' && formdata['poll_type'] == 'popover') || (formdata['form_type'] == 'calculator' && formdata['poll_type'] == 'popover')  || (formdata['form_type'] == 'facebook' && formdata['poll_type'] == 'popover') || (formdata['form_type'] == 'twitter' && formdata['poll_type'] == 'popover') || (formdata['form_type'] == 'pinterest' && formdata['poll_type'] == 'popover') || (formdata['form_type'] == 'action' && formdata['poll_type'] == 'popover') || (formdata['form_type'] == 'push_notification' && formdata['poll_type'] == 'popover'))
			{
				add_block_page();
			...
			if((isMobile && formdata["form_type"] == 'popover') || (isMobile && formdata["poll_type"] == 'popover'))
			{
				$('.optin_modal_box').css('width', '90%');
			}

			if(isMobile && formdata["form_type"] == 'scrollbox')
			{
				$('.optin_modal_box').css('width', '100%');
			}
			...
			{
				$('.optin_modal_box .background_image').css('background-image',"url(" + "http://localhost:8000" + "/" + formdata['background_image'] + ")");
					...
					if(questions.length > 0)
					{
						...
						$.each(questions[current_index]['options'], function(index, value){
							if (questions[current_index]['options_type'] == "image")
							{
								...
								{
									$('.optin_modal_box #question_preview #option_preview').append('<div class="ui radio checkbox field"><input type="radio" name="option" value="' + value['id'] + '" data-nextquestionid="' + value['next_question_id'] + '" data-optionvalue="' + value['option_value'] + '" required="required" ><label><img src="http://localhost:8000/optionimages/defaultoption.png" /></label></div>');
								}
								else
								{
									$('.optin_modal_box #question_preview #option_preview').append('<div class="ui radio checkbox field"><input type="radio" name="option" value="' + value['id'] + '" data-nextquestionid="' + value['next_question_id'] + '" data-optionvalue="' + value['option_value'] + '" required="required" ><label><img src="http://localhost:8000/' + value['image_url'] +'" /></label></div>');
								}
							}
							else
							{
	...
	function showOutcomeResult(answer_storage){
	    $.ajax({
	        dataType: "json",
	        data: answer_storage,
	        url: 'http://localhost:8000/api/v1/outcome',
	        method: 'post',
	        success: function(data){
	        	console.log(data);
	        	$('.optin_modal_box #subscribe_preview').remove();
	...
	$(document).on('click','.optin_modal_box #form_preview #start_action_button', function(e){
		...
		$.ajax({
			url : 'http://localhost:8000/api/v1/clicks',
			method : 'post',
			data : fdata,
			success: function(data){
				console.log('success');
