...
class CalculatorController extends Controller
{
    ...
    public function userCalculators()
    {
        ...
        $calculators = $user->currentAccount->forms->where('form_type', 'calculator')->where('is_template', false);

        $templates = Form::where('form_type', 'calculator')->where('is_template', true)->get();
        ...
        return view('calculators.calculators')->with(['user' => $user, 'calculators' => $calculators, 'main_user' => $mainUser, 'templates' => $templates, 'labels' => $labels, 'subscriptions' => $subs, 'visits' => $vis, 'forms_count' => $forms_count, 'subs_count' => $subs_count, 'vis_count' => $vis_count, 'conversion_rate' => round($conversion_rate, 2)]);
    }
    ...
    public function createCalculator(Request $request)
    {
        ...
        $form->theme_id = Theme::where('form_type', 'calculator')->where('poll_type', $data['poll_type'])->first()->id;
        $form->sub_account_id = $user->currentAccount->id;
        $form->save();

        return redirect('createcalculator/' . $form->id);
    }

    public function viewCalculator($id)
    {
        $user = Auth::user();
        $mainUser = $user;
        if ($user->role_id == 3) {
        ...
        if ($form) {
            ...
            return view('calculators.createcalculator')->with(['user' => $user, 'form' => $form, 'themes' => $themes, 'current_theme' => $current_theme, 'rules' => $rules, 'questions' => $questions, 'results' => $results, 'main_user' => $mainUser]);
        }
        return back()->with(['error' => 'Calculator not found']);
    }

    public function getQuestionMaxValue($id)
    {
    ...
    public function getQuestionsAndResults($id)
    {
        $output = [];
        ...
        if ($form) {
            ...
            foreach ($questions as $key => $value) {
                $number = $key + 1;
                $output['Q' . $number] = $value->max_value;
            }
    ...
    public function editCalculator($id)
    {
        ...
        if ($form) {
            ...
            return view('calculators.editcalculator')->with(['user' => $user, 'form' => $form, 'themes' => $themes, 'current_theme' => $current_theme, 'rules' => $rules, 'questions' => $questions, 'results' => $results, 'main_user' => $mainUser]);
        }
        return back()->with(['error', 'Calculator not found']);
    }

    public function createCalculatorFromTemplate(Request $request)
    {
        $user = Auth::user();
        if ($user->role_id == 3) {
            $user = $user->userLeader;
        ...
        $template = Form::find($data['calculator_template']);

        $form = new Form();
        $form->title = $data['title'];
        $form->form_type = 'calculator';
        $form->poll_type = $data['calculator_type'];
        $form->theme_id = Theme::where('form_type', 'calculator')->where('poll_type', $data['calculator_type'])->first()->id;
        $form->sub_account_id = $user->currentAccount->id;
        $form->page_load = true;
        ...
        if($template)
        {
            ...
            foreach($template->results as $t_result)
            {
                $result = new Result();
                $result->form_id = $form->id;;
                $result->formular = $t_result->formular;
                $result->save();
            }
            ...
            foreach($template->questions as $question)
            {
                ...
                foreach($question->options as $t_option)
                {
                    ...
                    $option->option_value = $t_option->option_value;
        ...
        return redirect('editcalculator/' . $form->id);
    }
    ...
    public function calculatorsAnalysis(Request $request)
    {
        $user = Auth::user();
        $data = $request->all();

        $forms = $user->currentAccount->forms->where('form_type', 'calculator')->where('is_template', false);
        $form_array = [];
