...
@section('title', 'Sendmunk | Calculators')
...
@section('content')
    <div class="ui vertical segment">
        <a id="create_template_icon" href="#" class="publish right floated ui button"><i class="plus icon"></i>Create Calculator from Template</a>
        <a id="create_calculator_icon" href="#" class="publish right floated ui button"><i class="plus icon"></i>Create New Calculator</a>
        <h2>Calculators</h2>
    </div>
    <br/>
    ...
    <table class="ui table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align: center">
                    <a href="{{ url('editcalculator').'/'.$form->id }}"><i data-content="Edit Calculator" class="edit icon"></i></a>
                    <a href="{{ url('delete/form').'/'.$form->id }}"><i data-content="Delete Calculator" class="delete icon"></i></a>
    ...
    @else
    <div class="ui message"><p>You don't have any Calculator(s) yet</p></div>
    @endif

    {{-- Create New Form Modal --}}
    <div id="create_calculator_modal" class="ui tiny modal">
        <div class="header">Create New Calculator</div>
        <div class="content">
            <form action="{{ url('createcalculator') }}" id="create_new_calculator" class="ui form" method="post" role="form">
                {{ csrf_field() }}
                <div class="field">
                ...
                <div class="field">
                    ...
                    <select name="poll_type" id="poll_type" class="ui dropdown" required>
    ...
    <div id="create_template_modal" class="ui tiny modal">
        <div class="header">Create New Calculator From Template</div>
        <div class="content">
            <form action="{{ url('createcalculatortemplate') }}" id="create_calculator_template" class="ui form" method="post" role="form">
                {{ csrf_field() }}
                <div class="field">
                    <label>Calculator Title</label>
                    <input id="calculator_title_template" type="text" name="title" required/>
                </div>
                <div class="field">
                    <label>Calculator Type</label>
                    <select name="calculator_type" id="calculator_type_template" class="ui dropdown" required>
                        <option value="">Select Calculator Type</option>
                        <option value="popover">Popover</option>
                        <option value="welcome_mat">Welcome Mat</option>
                    </select>
                ...
                <div class="field">
                    <label>Calculator Template(s)</label>
                    <select name="calculator_template" id="calculator_template" class="ui dropdown" required>
                        <option value="">Select Calculator Template</option>
        ...
        <div class="actions">
            <div class="ui cancel button">Cancel</div>
            <input id="create_calculator_temp_btn" type="submit" value="Create" form="create_calculator_template" class="ui primary button" />
        </div>
    </div>
...
@section('footerscripts')
    ...
    <script>
        $('#create_calculator_icon').on('click', function(){
            ...
            $('#calculator_type').dropdown('clear');
            $('#create_calculator_modal').modal('show');
        });

        $('#create_template_icon').on('click', function(){
            $('#calculator_title_template').val('');
            $('#calculator_type_template').dropdown('clear');
            $('#calculator_template').dropdown('clear');
