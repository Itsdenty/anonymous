...
@section('title', 'Sendmunk | Edit Campaign')
...
@section('content')
    ...
    <div class="ui pointing secondary stackable menu">
        ...
        <a id="content_tab" class="item" data-tab="content">Editor</a>
    ...
    <form id="campaign_form" action="{{ url('updatecampaign') }}" method="POST" role="form">
                ...
                <div class="ten wide column ui segment">
                    <div class="ui form">
                        ...
                        <div class="field">
                            ...
                            <select id="integration" class="ui dropdown" name="integration">
                    ...
                    <div class="ui form">
                        <div class="ui segment">
                            <div style="width: 30%" class="ui slider checkbox">
                                <input type="checkbox" name="ab_test" id="ab_test" {{ $campaign->ab_test ? 'checked' : '' }} />
                                <label>AB Test</label>
                            </div>
                            <br/><br/>
                            <div class="equal field">
                                <label id='label'>Email Subject</label>
                                <div id='div' class="ui right labeled input">
                                    <input data-emojiable="true" id="subject_a" type="text" placeholder="Enter Subject" name="subject_a" value="{{ $campaign->subject_a }}" />
                                ...
                                <div id="subject_b_div">
                                    ...
                                    <div class="ui right labeled input">
                                        <input id="subject_b" type="text" placeholder="Enter Subject 2" name="subject_b" value="{{ $campaign->subject_b }}" />
                                        <div class="ui dropdown label">
                                            <div class="text">Personalize</div>
                                            <i class="dropdown icon"></i>
                                            <div class="menu">
        ...
        <div class="ui tab" data-tab="preview">
            <div class="ui stackable grid">
                ...
                <div class="six wide column">
                    <div class="ui form">
                        {{-- <input id="send_later" name="send_later" type="checkbox" {{ $campaign->status == 'later' ? 'checked' : '' }} hidden/> --}}
                        <div class="field">
                            @if($campaign->status == 'draft')
                            <label>This campaign is currently a draft, when sent it would be delivered to <span class="campaign_contacts_count">{{ $contacts->count() }}</span> recepients</label>
                            @elseif($campaign->status == 'later')
                            ...
                            @endif
                            <br/>
                            <label><i class="mail icon"></i> Status: {{ $campaign->status }}</label>
                            <br/>
                            ...
                            <input type="text" value="{{ $campaign->send_date }}" name="schedule_date" id="schedule_date" readonly hidden/>
                            <br/><br/> --}}

                            <div class="grouped fields">
                                <div class="field">
                                    <div class="ui radio checkbox">
                                        <input type="radio" name="send_later" value="0" required/>
                                        <label><strong>Send it now</strong><br/><p>Send your campaign now</p></label>
                                    </div>
                                </div>
                                <div class="field">
                                    <div class="ui radio checkbox">
                                        <input type="radio" name="send_later" value="1" {{ $campaign->status == 'later' ? 'checked' : '' }} required />
                                        <label>
                        ...
                        <br/><br/><br/>
                        <a href="{{ url('duplicate/'.$campaign->id.'/campaign') }}" class="ui fluid button">Duplicate Campaign</a>
                    </div>
                </div>
            </div>
            <hr/>
...
@section('footerscripts')
    ...
    <script>
        ...
        //         $('#schedule_link').hide();
        //         $('#later_link').show();
        //         $('#send_label').text('at ' + $('#schedule_date').val());
        ...
        $(document).ready(function(){
            // set campaign values
            $('#email-content').summernote('code', {!! json_encode($campaign->content) !!});
            let from_reply_id = "{{ $campaign->from_reply_id }}";
            let smtp_id = "{{ $campaign->smtp_id }}";
            let api_id = "{{ $campaign->mail_api_id }}";

            if(from_reply_id)
            {
                $("#from_reply_email").val('{{ $campaign->from_reply_id }}').change();
            }
            if(smtp_id || api_id)
            {
                $("#integration").val('{{ $campaign->smtp_id ? "smtp_".$campaign->smtp_id : ($campaign->mail_api_id ? "api_".$campaign->mail_api_id : "") }}').change();
            }
            ...
            $('#send_later').trigger('change');
            $("input[type='radio'][name='send_later'][value='{{ $campaign->status == 'later' ? '1' : '0' }}']").trigger('change');
