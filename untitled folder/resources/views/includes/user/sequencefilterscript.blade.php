...
// Filter component
    ...
    $('#filter_campaign_btn').on('click', function(e){
        ...
        if(query.length != 0)
        {
            ...
            let filter_url = "{{ url('contacts/filters').'?' }}" + query.join('&') + "&order_column=sub_date&order_direction=desc&filter_match="+ match_value;
            $.ajax({
                url: filter_url,
                method: 'get',
    ...
    $('#filter_reset_btn').on('click', function(e){
        ...
        $.ajax({
            url: "{{ url('contacts/filters').'?' }}",
            method: 'get',
            success: function(response){
                contacts_data = response.collection.data;
                filter_query_string = "";
                $.ajax({
                    url: "{{ url('contacts/sequence').'/'.$sequence->id }}",
                    method: 'post',
                    data: {'campaign_contacts' : contacts_data, 'filter_query_string' : filter_query_string,  _token: '{{ csrf_token() }}'},
                    success:function(response){
    ...
    $('#apply_filter').on('click', function(e){
        ...
        $.ajax({
            url: "{{ url('contacts/sequence').'/'.$sequence->id }}",
            method: 'post',
            data: {'sequence_contacts' : contacts_data, 'filter_query_string' : filter_query_string, _token: '{{ csrf_token() }}'},
            success:function(response){
    ...
    function updateSubscribersList()
    {
        ...
        {
            output = '<div class="ui message">You have not contacts for this campaign</div>';
        }
        else
        {
            output = '<table class="ui table"><thead><tr><th width="40%">Name</th><th width="40%">Email</th><th style="text-align: center">Action</th></tr></thead><tbody>';
            
            $.each(contacts_data, function(index, value){
                output += '<tr><td>' + value.name + '</td><td>' + value.email + '</td><td style="text-align: center"><a class="remove_contact" href="{{ url("remove_contact") }}' +'/'+ value.id + '/sequence' + '/' + '{{$sequence->id}}' + '"><i class="delete icon"></i></a></td></tr>';
            });
            output += '</tbody></table>';
        }
        $('#create_form_modal_6 .content').empty();
        $('#create_form_modal_6 .content').append(output);
    }
</script>