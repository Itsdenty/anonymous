    ...
    $('#filter_reset_btn').on('click', function(e){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $.ajax({
                    url: "{{ url('contacts/campaign').'/'.$campaign->id }}",
                    method: 'post',
    ...
    function updateSubscribersList()
    {
        ...
        else
        {
            ...
            $.each(contacts_data, function(index, value){
                output += '<tr><td>' + value.name + '</td><td>' + value.email + '</td><td style="text-align: center"><a class="remove_contact" href="{{ url("remove_contact") }}' +'/'+ value.id + '/campaign' + '/' + '{{$campaign->id}}' + '"><i class="delete icon"></i></a></td></tr>';
            });
            output += '</tbody></table>';
        }
        $('#create_form_modal_6 .content').empty();
