...
// Filter component
    ...
    $('#create_segment_icon').on('click', function(e){
        $('#filter_reset_btn').trigger('click');
    ...
    $('#apply_filter').on('click', function(e){
        ...
        $('.filter_ctrl').prop('disabled', true);
        let segment_name = $('#segment_input_name').val();

        $.ajax({
            url: "{{ url('contacts/segment/save') }}",
            method: 'post',
            data: {'segment_contacts' : contacts_data, 'name' : segment_name, 'filter_query_string' : filter_query_string, _token: '{{ csrf_token() }}'},
            success:function(response){
    ...
    $('#segment_input_name').on("input", function(){
        if($(this).val() != "")
        {
