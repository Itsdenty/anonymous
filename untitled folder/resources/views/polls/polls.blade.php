...
@section('content')
    <div class="ui vertical segment">
        <a id="create_template_icon" href="#" class="publish right floated ui button"><i class="plus icon"></i>Create Poll from Template</a>
        <a id="create_poll_icon" href="#" class="publish right floated ui button"><i class="plus icon"></i>Create New Poll</a>
        <h2>Polls</h2>
    </div>
    <br/>
    ...
    <table class="ui table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align: center">
                    <a href="{{ url('editpoll').'/'.$form->id }}"><i data-content="Edit Poll" class="edit icon"></i></a>
                    <a href="{{ url('delete/form').'/'.$form->id }}"><i data-content="Delete Poll" class="delete icon"></i></a>
    ...
    @else
    <div class="ui message"><p>You don't have any Poll(s) yet</p></div>
    @endif
    ...
    <div id="create_poll_modal" class="ui tiny modal">
        ...
        <div class="content">
            <form action="{{ url('createpoll') }}" id="create_new_poll" class="ui form" method="post" role="form">
                {{ csrf_field() }}
                <div class="field">
    ...
    <div id="create_template_modal" class="ui tiny modal">
        <div class="header">Create New Poll From Template</div>
        <div class="content">
            <form action="{{ url('createpolltemplate') }}" id="create_poll_template" class="ui form" method="post" role="form">
                {{ csrf_field() }}
                <div class="field">
                    <label>Poll Title</label>
                    <input id="poll_title_template" type="text" name="title" required/>
                </div>
                <div class="field">
                    <label>Poll Type</label>
                    <select name="poll_type" id="poll_type_template" class="ui dropdown" required>
                        <option value="">Select Poll Type</option>
                        <option value="popover">Popover</option>
                        <option value="welcome_mat">Welcome Mat</option>
                    </select>
                ...
                <div class="field">
                    <label>Poll Template(s)</label>
                    <select name="poll_template" id="poll_template" class="ui dropdown" required>
                        <option value="">Select Poll Template</option>
        ...
        <div class="actions">
            <div class="ui cancel button">Cancel</div>
            <input id="create_poll_temp_btn" type="submit" value="Create" form="create_poll_template" class="ui primary button" />
        </div>
    </div>
...
@section('footerscripts')
    ...
    <script>
        $('#create_poll_icon').on('click', function(){
            $('#poll_title').val('');
            $('#poll_type').dropdown('clear');
            $('#create_poll_modal').modal('show');
        });

        $('#create_template_icon').on('click', function(){
            $('#poll_title_template').val('');
            $('#poll_type_template').dropdown('clear');
            $('#poll_template').dropdown('clear');
