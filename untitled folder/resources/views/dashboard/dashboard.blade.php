...
@section('title', 'Sendmunk | Dashboard')
...
@section('content')
    ...
    <div class="ui vertically stackable grid">
        <div class="four column row">
            ...
            <div class="summary column">
                ...
                <h2>{{ $num_of_opens }} / {{ $num_of_contacts == 0 ? 0 : number_format((float)$num_of_opens / $num_of_contacts * 100, 2, '.', '')}}%</h2>
                <h3>No. of Openers</h3>
            </div>
            <div class="summary column">
                <h3><i class="hand pointer outline icon"></i></h3>
                <h2>{{ $num_of_clicks }} / {{ $num_of_contacts == 0 ? 0 : number_format((float)$num_of_clicks / $num_of_contacts * 100, 2, '.', '')}}%</h2>
    ...
    <br/>
    <div class="ui segment">
        @if(!$sms_campaigns->isEmpty())
        <h3>Recent SMS/MMS Campaigns</h3>
        <table class="ui table">
            ...
            <tbody>
                @foreach($sms_campaigns as $campaign)
                <tr>
                    <td><a href="{{ ($campaign->status == 'draft' || $campaign->status == 'later') ? url('edit/'.$campaign->id.'/smscampaign') : url('view_analysis_sms').'/'.$campaign->id}}"><h4>{{ $campaign->title }}</h4></a>{{ $campaign->status != 'draft' ? 'Sent Date: '.(new DateTime($campaign->send_date))->format('M j, Y H:i:s') : 'Created: '.(new DateTime($campaign->created_at))->format('M j, Y H:i:s') }}</td>
                    <td style="text-align: center">{{ $campaign->links->map(function($link){return $link->contacts;})->flatten()->unique('id')->count() }} / {{ $campaign->links->map(function($link){return $link->contacts;})->flatten()->unique('id')->count() == 0 || $campaign->contacts()->wherePivot('sent', true)->count() == 0 ? '0' :  number_format((float)$campaign->links->map(function($link){return $link->contacts;})->flatten()->unique('id')->count() / $campaign->contacts()->wherePivot('sent', true)->count() * 100, 2, '.', '')}}%</td>
                    <td style="text-align: center">{{ $campaign->contacts()->wherePivot('sent', true)->count() }}/{{ $campaign->contacts->count() }}</td>
                    <td style="text-align: center">{{ ucfirst($campaign->status) }}</td>
                    {{-- <td style="text-align: center">
                        <a href="{{ url('view_analysis_sms').'/'.$campaign->id }}"><i data-content="Campaign Report" class="assistive listening systems icon"></i></a>
                        <a href="{{ url('duplicate/'.$campaign->id.'/smscampaign') }}"><i data-content="Duplicate Campaign" class="copy icon"></i></a>
                        @if($campaign->status == 'draft' || $campaign->status == 'later')
                        <a href="{{ url('edit/'.$campaign->id.'/smscampaign') }}"><i data-content="Edit Campaign" class="edit icon"></i></a>
                        @endif
                        @if($campaign->status != 'progress')
                        <a href="{{ url('delete/'.$campaign->id.'/smscampaign') }}"><i data-content="Delete Campaign" class="delete icon"></i></a>
                        @endif
                    </td> --}}
                </tr>
                @endforeach
        ...
        @else
        <div class="ui message"><p>You don't have any SMS/MMS Campaign(s) yet</p></div>
        @endif
    </div>
@endsection
